...
def embed_message(
    cover_path, output_path, message, password, decoy_message=None, decoy_password=None
):
    ...
    if decoy_message and decoy_password:
        _embed_payload(
            img, _build_payload_bytes(decoy_message, decoy_password), decoy_password, DECOY_CHANNEL
        )
...
def remove_exif(image_path, output_path):
    ...
    try:
        ...
        raise ValueError(f"Failed to remove EXIF: {e}") from e
