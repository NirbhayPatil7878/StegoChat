...
PASSWORD_PATTERNS = [
    ...
    (
        re.compile(r"\botp\b|\bone[- ]?time\b|\bverification code\b", re.I),
        "Contains OTP-related phrase",
    ),
...
def extract():
    ...
    try:
        ...
        if not password:
            os.remove(tmp_path)
            return jsonify({"error": "No hidden data found or password required"}), 400
        ...
        try:
            ...
            try:
                ...
                if has_marker:
                    ...
                    os.remove(tmp_path)
                    return jsonify({"type": "text", "data": message})
...
def scan_message_risk(message: str):
    ...
    if any(
        "secret key" in item.lower() or "token" in item.lower() or "wallet" in item.lower()
...
def inspect_image_file(file_storage):
    ...
    try:
        ...
        if pixels:
            lsb_ones = sum(
                (r & 1) + (g & 1) + (b & 1) for r, g, b in pixels[: min(len(pixels), 50000)]
            )
...
def api_fetch_dead_drop(token):
    ...
    return jsonify(
        {
            "status": "ok",
            "message": drop.get("message", ""),
            "createdAt": drop.get("createdAt"),
...
def update_settings():
    ...
    try:
        ...
        if (
            "encryption_protocol" in new_settings
            and new_settings["encryption_protocol"] not in valid_protocols
        ):
            return jsonify({"error": "Invalid encryption protocol"}), 400

        if (
            "embedding_method" in new_settings
            and new_settings["embedding_method"] not in valid_methods
        ):
...
def update_profile():
    ...
    try:
        ...
        if "username" in new_profile and (
            len(new_profile["username"]) < 3 or len(new_profile["username"]) > 50
        ):
        ...
        if stored_password_hash:
            ...
            try:
                if not bcrypt.checkpw(
                    old_password.encode("utf-8"), stored_password_hash.encode("utf-8")
                ):
